...
func (r *mcsAPIEndpointSliceMirrorReconciler) getDerivedService(ctx context.Context, derivedEpSlice *discoveryv1.EndpointSlice) (*corev1.Service, error) {
	...
	if err := r.Client.Get(
		ctx,
		types.NamespacedName{Name: derivedServiceName, Namespace: derivedEpSlice.Namespace},
		&derivedService,
	); err != nil {
...
// getLocalEndpointSliceIfExported returns a similar local exported
// EndpointSlice as the given derived derived EndpointSlice
func (r *mcsAPIEndpointSliceMirrorReconciler) getLocalEndpointSliceIfExported(
	...
	if err := r.Client.Get(
		ctx,
		types.NamespacedName{Name: serviceName, Namespace: derivedEpSlice.Namespace},
		&svcExport,
	); err != nil {
		return nil, client.IgnoreNotFound(err)
	}
...
// getLocalDerivedEndpointSlice returns a similar derived EndpointSlice as the
// given local given EndpointSlice
...
func (r *mcsAPIEndpointSliceMirrorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if epSlice.Labels != nil && epSlice.Labels[discoveryv1.LabelManagedBy] == endpointSliceLocalMCSAPIControllerName {
		derivedEpSlice = &epSlice
		derivedService, err = r.getDerivedService(ctx, derivedEpSlice)
		if err != nil {
			return controllerruntime.Fail(err)
		}
		localEpSlice, err = r.getLocalEndpointSliceIfExported(ctx, derivedEpSlice, derivedService)
	} else {
	...
	} else if r.needUpdate(localEpSlice, derivedEpSlice, derivedService) {
...
func (r *mcsAPIEndpointSliceMirrorReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		...
		// Watch for changes to Service to enqueue derived EndpointSlice
		Watches(&corev1.Service{}, handler.EnqueueRequestsFromMapFunc(func(ctx context.Context, obj client.Object) []ctrl.Request {
			...
			if svcImportOwner == "" {
				serviceReq, _ := labels.NewRequirement(mcsapiv1alpha1.LabelServiceName, selection.Equals, []string{obj.GetName()})
