		...
		cache.ResourceEventHandlerFuncs{
			AddFunc: func(obj interface{}) {
				if service := k8s.CastInformerEvent[v1.Service](obj); service != nil {
					...
					log.WithField(logfields.ServiceKey, key).Debug("Queuing wide service update after Kubernetes service update")
...
func (c *EndpointSliceController) onClusterAdd(cluster string) {
	log.WithField(logfields.ClusterName, cluster).Debug("Creating endpointslice tracker cluster entry")
...
func (c *EndpointSliceController) onClusterDelete(cluster string) {
	log.WithField(logfields.ClusterName, cluster).Debug("Removing endpointslice tracker cluster entry")
...
func (c *EndpointSliceController) onClusterServiceUpdate(clusterSvc *serviceStore.ClusterService) {
	...
	log.WithFields(logrus.Fields{
		logfields.ClusterName: clusterSvc.String(),
		logfields.ServiceKey:  key,
	}).Debug("Queuing cluster service update after global service update")
...
func (c *EndpointSliceController) queueServiceForEndpointSlice(endpointSlice *discoveryv1.EndpointSlice) {
	...
	if !ok || clusterName == "" {
		log.WithFields(logrus.Fields{
			logfields.K8sEndpointName: endpointSlice.Name,
			logfields.ServiceKey:      key,
		}).Debug("Queuing wide service update for an endpointslice")
	...
	}).Debug("Queuing cluster service update for an endpointslice")
...
func (c *EndpointSliceController) processEvent() bool {
	...
	if err == nil {
		...
		log.WithError(err).Warnf("Failed to process EndpointSlice event, requeing: %s", event)
...
func (c *EndpointSliceController) handleEvent(event interface{}) error {
	...
	case serviceSyncEvent:
		log.WithField(logfields.ServiceKey, ev.key).Debug("Handling service sync event")
		err = c.handleServiceSyncEvent(ev.key, nil)
	case serviceClusterSyncEvent:
		log.WithFields(logrus.Fields{
			logfields.ClusterName: ev.cluster,
			logfields.ServiceKey:  ev.key,
		}).Debug("Handling cluster service sync event")
...
func (c *EndpointSliceController) deleteRemoteEndpointSlicesForCluster(service *v1.Service, cluster string) error {
...
func (c *EndpointSliceController) deleteRemoteEndpointSlices(service *v1.Service) error {
	return c.clientset.DiscoveryV1().EndpointSlices(service.Namespace).DeleteCollection(
		context.Background(),
		metav1.DeleteOptions{},
		metav1.ListOptions{
...
func (c *EndpointSliceController) handleServiceSyncEvent(key types.NamespacedName, cluster *string) error {
	...
	if !getAnnotationIncludeExternal(service) || !getAnnotationSyncEndpointSlices(service) {
		c.triggerTimeTracker.DeleteService(key)
		c.reconciler.DeleteService(key.Namespace, key.Name)
		for _, endpointSliceTracker := range c.endpointSliceTrackers {
			endpointSliceTracker.DeleteService(key.Namespace, key.Name)
		}
		// The service no longer synchronize his endpoint slices, so we make sure
		// all the multicluster endpointslices are deleted
		return c.deleteRemoteEndpointSlices(service)
	}
	...
	for cluster := range clusters {
		...
		if !ok || endpointSliceTracker == nil {
			...
			c.deleteRemoteEndpointSlicesForCluster(service, cluster)
			return nil
		}
		...
		log.WithFields(logrus.Fields{
			logfields.ServiceKey:  key,
			logfields.ClusterName: cluster,
		}).Debugf("Reconciling endpointslices of ClusterService %#v", clusterSvc.svc)
...
func getAnnotationSyncEndpointSlices(svc *v1.Service) bool {
	if value, ok := annotation.Get(svc, annotation.GlobalServiceSyncEndpointSlices); ok {
		return strings.ToLower(value) == "true"
	}
