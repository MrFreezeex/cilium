...
func IsBackendReferenceAllowed(originatingNamespace string, be gatewayv1beta1.BackendRef, gvk schema.GroupVersionKind, grants []gatewayv1beta1.ReferenceGrant) bool {
...
func IsSecretReferenceAllowed(originatingNamespace string, sr gatewayv1beta1.SecretObjectReference, gvk schema.GroupVersionKind, grants []gatewayv1beta1.ReferenceGrant) bool {
...
func isReferenceAllowed(originatingNamespace, name string, namespace *gatewayv1beta1.Namespace, fromGVK, toGVK schema.GroupVersionKind, grants []gatewayv1beta1.ReferenceGrant) bool {
	...
	for _, g := range grants {
		...
		for _, from := range g.Spec.From {
			if (from.Group == gatewayv1beta1.Group(fromGVK.Group) && from.Kind == gatewayv1beta1.Kind(fromGVK.Kind)) &&
				(string)(from.Namespace) == originatingNamespace {
				for _, to := range g.Spec.To {
					if to.Group == gatewayv1beta1.Group(toGVK.Group) && to.Kind == gatewayv1beta1.Kind(toGVK.Kind) &&
