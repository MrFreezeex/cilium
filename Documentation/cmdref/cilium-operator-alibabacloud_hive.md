...
## cilium-operator-alibabacloud hive

Inspect the hive

```
cilium-operator-alibabacloud hive [flags]
```
...
```
      ...
      --cluster-mesh-endpoint-updates-batch-period duration   Describes the length of endpoint updates batching period (default 100ms)
      --cluster-mesh-endpoints-per-slice int                  Maximum number of endpoints that will be added to an endpoint slice synchronized from other clusters in the mesh (default 100)
      --cluster-name string                                   Name of the cluster (default "default")
      --clustermesh-config string                             Path to the ClusterMesh configuration directory
      --concurrent-service-endpoint-syncs int                 Number of service endpoint syncing operations that will be done concurrently for  (default 5)
...
* [cilium-operator-alibabacloud hive dot-graph](cilium-operator-alibabacloud_hive_dot-graph.md)	 - Output the dependencies graph in graphviz dot format
